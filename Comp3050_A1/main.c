...
int main(int argc, char* argv[])
{
	...
	for (int x = 10; x != 0; x--) {
		...
		scanf_s("%g", &float_32.floating_value_in_32_bits);
		...
		printf("The floating point value for %g is broken out as:\n", float_32.floating_value_in_32_bits);
		printf("     mantissa: 0x%x or:             %s\n", float_32.f_bits.mantissa, bit_string + strlen(bit_string) - 30);
		printf("     exponent: 0x%x     or:  %.11s\n", float_32.f_bits.exponent, bit_string + 1);
		printf("         sign: %x        or: %d\n", float_32.bit.b31, float_32.bit.b31);
		printf("   in base 10: %d or: %s\n", float_32.floating_value_as_int, bit_string);
	}
}